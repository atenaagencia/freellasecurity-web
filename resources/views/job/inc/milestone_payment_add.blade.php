...
@section('title-page')
<h1 class="h3 py-4 font-weight-bold text-dark">{{__('Add Payment for Milestones')}}</h1>
@endsection
	...
	<div class="col-md-12 py-2">
		<div class="card shadow">
			...
			<div class="card-header bg-white rounded-0 py-3">
				<h6 class="m-0 font-weight-bold text-dark">{{__('Add Payment for Create new Milestone :')}}</h6>
			</div>
			<!-- Card Body -->
			<div class="card-body bg-white rounded-0">
				<div class="formpanel">
					...
					<div class="row">
						...
						<div class="col-md-6 py-2">
							<div class="formrow">
								...
								<select class="form-control" name="ccExpiryMonth" id="ccExpiryMonth">
									@for ($counter = 1; $counter <= 12; $counter++) @php $val=str_pad($counter, 2, '0' , STR_PAD_LEFT); @endphp <option value="{{$val}}">{{$val}}</option>
										@endfor
								</select>
						...
						<div class="col-md-12 py-2">
							<div class="formrow">
								<button type="submit" class="btn btn-success">{{__('Pay')}} <i class="fa fa-arrow-circle-right" aria-hidden="true"></i></button>
							</div>
						</div>
					</div>
					{!! Form::close() !!}
