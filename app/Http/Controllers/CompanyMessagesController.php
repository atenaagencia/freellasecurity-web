...
class CompanyMessagesController extends Controller
{
    ...
    public function candidatePayments() {
        $companyId = Auth::guard('company')->user()->id;
        $users = FavouriteApplicant::where('company_id', $companyId)
                                    ->groupBy('user_id')
                                    ->paginate(5);
        ...
        foreach ($users as $user) {
            ...
            $userDetail = User::where('id', $candidateId)->get();  
            if(!empty($userDetail->toArray())){
               /* Hired Candidates names */
               $firstName =  $userDetail[0]->first_name;
               $middleName =  $userDetail[0]->middle_name;
               $lastName =  $userDetail[0]->last_name;
               $user->name = $firstName.' '.$middleName.' '.$lastName;
               
               /* Hired candidates projects */
               

               $jobDetails = Milestones::where('candidate_id', $candidateId)
               ...
               if(!empty($jobDetails->toArray())){
                    foreach ($jobDetails as $jobDetail) {
                       ...
                       $singleJob = Job::where('id', $jobId)->get();
                       if(!empty($singleJob->toArray())){
                            $singleJob = $singleJob->toArray()[0];
                            array_push($jobs, $singleJob);
                       }
                       
                    }
                    $user->jobDetails = $jobs;
               }
            }                   
            
        }                                    

        return view('company.hiredcandidates')->with('candidates', $users);
    }
