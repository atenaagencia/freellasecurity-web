            ...
            <div class="col-md-12"> 
                ...
                <div class="portlet light portlet-fit portlet-datatable bordered">
                    <div class="portlet-title">
                        ...
                        <div class="actions"> <a href="<?php echo e(route('create.video')); ?>" class="btn btn-xs btn-success"><i class="glyphicon glyphicon-plus"></i> Add New Video language</a> </div>
                    </div>
...
<script>
    $(function () {
        var oTable = $('#videoDatatableAjax').DataTable({
             ...
             */
            ajax: {
                url: '<?php echo route('fetch.data.videos'); ?>',
                data: function (d) {
                    d.lang = $('#lang').val();
    ...
    function deleteVideo(id, is_default) {
        ...
        if (confirm(msg)) {
            $.post("<?php echo e(route('delete.video')); ?>", {id: id, _method: 'DELETE', _token: '<?php echo e(csrf_token()); ?>'}, null, 'text')
    ...
    function makeActive(id) {
        $.post("<?php echo e(route('make.active.video')); ?>", {id: id, _method: 'PUT', _token: '<?php echo e(csrf_token()); ?>'})
                .done(function (response) {
                    if (response == 'ok')
                    {
                        var table = $('#videoDatatableAjax').DataTable();
    ...
    function makeNotActive(id) {
        $.post("<?php echo e(route('make.not.active.video')); ?>", {id: id, _method: 'PUT', _token: '<?php echo e(csrf_token()); ?>'})
                .done(function (response) {
                    if (response == 'ok')
                    {
                        var table = $('#videoDatatableAjax').DataTable();
