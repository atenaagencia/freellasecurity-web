            ...
            <div class="col-md-12"> 
                ...
                <div class="portlet light portlet-fit portlet-datatable bordered">
                    <div class="portlet-title">
                        ...
                        <div class="actions"> <a href="<?php echo e(route('create.language')); ?>" class="btn btn-xs btn-success"><i class="glyphicon glyphicon-plus"></i> Add New Language</a> </div>
                    </div>
...
<script>
    $(function () {
        var oTable = $('#languageDatatableAjax').DataTable({
             ...
             */
            ajax: {
                url: '<?php echo route('fetch.data.languages'); ?>',
                data: function (d) {
                    d.lang = $('#lang').val();
                    d.native = $('#native').val();
    ...
    function deleteLanguage(id, is_default) {
        ...
        if (is_default != 1) {
            if (confirm(msg)) {
                $.post("<?php echo e(route('delete.language')); ?>", {id: id, _method: 'DELETE', _token: '<?php echo e(csrf_token()); ?>'})
    ...
    function makeDefaultLanguage(id) {
        $.post("<?php echo e(route('make.default.language')); ?>", {id: id, _method: 'PUT', _token: '<?php echo e(csrf_token()); ?>'})
                .done(function (response) {
                    if (response == 'ok')
                    {
                        var table = $('#languageDatatableAjax').DataTable();
    ...
    function makeActive(id) {
        $.post("<?php echo e(route('make.active.language')); ?>", {id: id, _method: 'PUT', _token: '<?php echo e(csrf_token()); ?>'})
                .done(function (response) {
                    if (response == 'ok')
                    {
                        var table = $('#languageDatatableAjax').DataTable();
    ...
    function makeNotActive(id) {
        $.post("<?php echo e(route('make.not.active.language')); ?>", {id: id, _method: 'PUT', _token: '<?php echo e(csrf_token()); ?>'})
                .done(function (response) {
                    if (response == 'ok')
                    {
                        var table = $('#languageDatatableAjax').DataTable();
